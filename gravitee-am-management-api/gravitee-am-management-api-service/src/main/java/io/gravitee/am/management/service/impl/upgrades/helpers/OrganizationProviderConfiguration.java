...
public abstract class OrganizationProviderConfiguration {
    ...
    public OrganizationProviderConfiguration(String type, Environment env, int index) {
        ...
        this.id = env.getProperty(propertyBase + "id", String.class);
        this.name = env.getProperty(propertyBase + "name", StringUtils.capitalize(this.type) + " users");
        this.refresh = env.getProperty(propertyBase + "refresh", boolean.class, false);
    }
    ...
    public abstract String generateConfiguration();

    public abstract String mergeConfiguration(String existingConfig);

    public abstract void upsertUsers(IdentityProvider provider);
